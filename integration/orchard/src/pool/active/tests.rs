...
fn physical_frames(journal: &ActiveJournal, genesis: &File) -> Result<Vec<u64>, PoolError> {
    ...
    reader.read_exact(&mut actual_header).map_err(|_| PoolError::Corrupt)?;
    ...
    while position < journal.length() {
        let start = position;
        let mut prefix = [0; 4];
        reader.read_exact(&mut prefix).map_err(|_| PoolError::Corrupt)?;
        let n = u32::from_be_bytes(prefix) as usize;
        frame_size(n as u64 + 36)?;
        let mut raw = vec![0; n];
        reader.read_exact(&mut raw).map_err(|_| PoolError::Corrupt)?;
        let mut digest = [0; 32];
        reader.read_exact(&mut digest).map_err(|_| PoolError::Corrupt)?;
...
fn create_is_exclusive_bounded_and_preserves_existing_bytes() {
    ...
    assert_eq!(physical_frames(&journal, &genesis).unwrap(), Vec::<u64>::new());
    assert!(matches!(ActiveJournal::open(&path, &header()), Err(PoolError::Locked)));
    ...
    incorrect[20] ^= 1;
    assert!(matches!(ActiveJournal::open(&path, &incorrect), Err(PoolError::Genesis)));
    ...
    #[cfg(unix)]
    {
        ...
        assert_eq!(fs::metadata(path.join(GENESIS)).unwrap().permissions().mode() & 0o777, 0o600);
    }
}
...
fn full_segment_and_next_frame_rotate_canonically_without_crossing() {
    let d = Dir::new();
    let path = d.path("ledger");
    let lengths = [MAX_FRAME_BYTES, MAX_FRAME_BYTES, SEGMENT_BYTES - 2 * MAX_FRAME_BYTES, MIN_FRAME_BYTES];
    ...
    assert_eq!(fs::metadata(path.join(name(0))).unwrap().len(), SEGMENT_BYTES);
    assert_eq!(fs::metadata(path.join(name(1))).unwrap().len(), MIN_FRAME_BYTES);
    assert_eq!(journal.capacity(), (76 + SEGMENT_BYTES + MIN_FRAME_BYTES, 2, MIN_FRAME_BYTES as u32));
    assert_eq!(physical_frames(&journal, &genesis).unwrap(), lengths);
    let mut actual = Vec::new();
    journal.reader(&genesis).unwrap().read_to_end(&mut actual).unwrap();
    assert_eq!(actual, expected);
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        assert_eq!(fs::metadata(path.join(name(0))).unwrap().permissions().mode() & 0o777, 0o600);
    }
    drop(journal);
    drop(genesis);
...
fn budgets_include_framing_total_tail_and_the_independent_segment_cap() {
    ...
    assert_eq!(frame_budget(TOTAL_BYTES - MIN_FRAME_BYTES, 1024, SEGMENT_BYTES - MIN_FRAME_BYTES, MIN_FRAME_BYTES), Ok(false));
    assert_eq!(frame_budget(TOTAL_BYTES - MIN_FRAME_BYTES + 1, 1024, SEGMENT_BYTES - MIN_FRAME_BYTES, MIN_FRAME_BYTES), Err(PoolError::Bounds));
    assert_eq!(frame_budget(76, 0, 0, MIN_FRAME_BYTES - 1), Err(PoolError::Bounds));
    assert_eq!(frame_budget(76, 0, 0, MAX_FRAME_BYTES + 1), Err(PoolError::Bounds));
    assert_eq!(frame_budget(76, 0, 0, u64::MAX), Err(PoolError::Bounds));
    assert_eq!(frame_budget(u64::MAX, 1, MIN_FRAME_BYTES, MIN_FRAME_BYTES), Err(PoolError::Bounds));
    assert_eq!(frame_budget(TOTAL_BYTES / 2, MAX_SEGMENTS, SEGMENT_BYTES - MIN_FRAME_BYTES, MIN_FRAME_BYTES), Ok(false));
    assert_eq!(frame_budget(TOTAL_BYTES / 2, MAX_SEGMENTS, SEGMENT_BYTES - MIN_FRAME_BYTES + 1, MIN_FRAME_BYTES), Err(PoolError::Bounds));
    assert_eq!(frame_budget(TOTAL_BYTES / 2, MAX_SEGMENTS - 1, SEGMENT_BYTES, MIN_FRAME_BYTES), Ok(true));
    for (count, tail) in [(0, 1), (1, 0), (1, SEGMENT_BYTES + 1), (MAX_SEGMENTS + 1, MIN_FRAME_BYTES)] {
        assert_eq!(frame_budget(76, count, tail, MIN_FRAME_BYTES), Err(PoolError::Bounds));
    }
}
...
fn unknown_missing_empty_and_truncated_segments_fail_closed_without_repair() {
    ...
    for (label, raw) in [
        ...
        ("trailing", { let mut raw = frame(400); raw.push(0); raw }),
    ] {
        let path = d.path(label);
        let (genesis, journal) = create(&path);
        drop(journal);
...
fn another_process_cannot_open_held_genesis_lock() {
    ...
    let probe = |held: bool| {
        ...
        command.args(["--exact", "pool::active::tests::another_process_cannot_open_held_genesis_lock", "--nocapture"]);
        ...
        assert!(output.status.success(), "lock subprocess failed: {} {}", String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));
...
fn captured_reader_requires_each_real_eof_and_poisoned_readers_cannot_resume() {
    ...
    for truncate in [false, true] {
        ...
        let external = OpenOptions::new().write(true).open(path.join(name(0))).unwrap();
...
fn unknown_entry_after_open_is_not_ignored_or_removed() {
    ...
    assert_eq!(fs::read(path.join("unexpected")).unwrap(), b"retain for inspection");
    assert_eq!(journal.capacity(), (76, 0, 0));
}
...
fn interruption_leaves_metadata_unchanged_and_preserves_the_recoverable_prefix() {
    ...
    for fault in 1..=4 {
        for existing in [false, true] {
            ...
            let before = journal.capacity();
            journal.fault = fault;
            assert_eq!(journal.append(&genesis, &frame(600)), Err(PoolError::Storage));
            assert_eq!(journal.capacity(), before);
            let raw = fs::read(path.join(name(0))).unwrap();
            if existing {
                assert_eq!(&raw[..first.len()], &first);
            }
            assert_eq!(raw.len(), if existing { 400 } else { 0 } + match fault { 1 => 0, 2 => 300, _ => 600 });
            assert!(journal.check(&genesis).is_err());
            drop(journal);
            drop(genesis);
            ...
            if fault == 1 {
                ...
                if fault == 2 {
                    ...
                    assert_eq!(scanned.unwrap(), if existing { vec![400, 600] } else { vec![600] });
                }
            }
        }
    }
...
fn failed_next_segment_publication_keeps_all_sealed_prefix_bytes_and_no_repair() {
    ...
    for fault in 1..=4 {
        ...
        let (genesis, mut journal) = create(&path);
        for length in [MAX_FRAME_BYTES, MAX_FRAME_BYTES, SEGMENT_BYTES - 2 * MAX_FRAME_BYTES] {
        ...
        journal.fault = fault;
        assert_eq!(journal.append(&genesis, &frame(MIN_FRAME_BYTES)), Err(PoolError::Storage));
        assert_eq!(journal.capacity(), before);
        assert_eq!(fs::read(path.join(name(0))).unwrap(), prefix);
        let next = fs::read(path.join(name(1))).unwrap();
        assert_eq!(next.len(), match fault { 1 => 0, 2 => MIN_FRAME_BYTES as usize / 2, _ => MIN_FRAME_BYTES as usize });
        assert!(journal.check(&genesis).is_err());
        drop(journal);
        drop(genesis);
        let reopened = ActiveJournal::open(&path, &header());
