...
fn injected_append_failures_preserve_error_semantics_and_do_not_pollute_success_metrics() {
    ...
    for point in 1..=4 {
        ...
        assert!(report
            .ordinary
            .iter()
            .chain(&report.rotating)
            .all(|m| m.count == 0));
        let failure = report.failure.unwrap();
        assert_eq!(failure.height, 1);
        if point == 1 {
            assert_eq!(failure.pending, Some(Phase::RotationPrepare));
        }
        if point == 2 {
            assert_eq!(failure.pending, Some(Phase::WriteFrame));
        }
        if point == 3 {
            assert!(failure.pending.is_none());
            assert_ne!(
                failure.completed_mask & (1 << Phase::WriteFrame as usize),
                0
            );
        ...
        if point == 4 {
            ...
            assert_eq!(
                failure.completed_mask & (1 << Phase::StatePublish as usize),
                0
            );
        }
        drop(pool);
...
fn assert_profile(r: &Snapshot, segments: u32) {
    ...
    assert_eq!(
        (r.attempts, r.accepted, r.failed, r.last_accepted, r.paid),
...
fn native_paired_commits() {
    ...
    for height in 1..=BLOCKS {
        assert!(
            ...
            "native profiling experiment exceeded budget"
        );
        ...
        if !txs.is_empty() {
            ...
            assert_eq!(
                pool.prepare(height + 1, id(height + 1), txs).err(),
    ...
    for height in 1..=BLOCKS {
        assert!(
            Instant::now() < deadline,
            "native profiling control exceeded budget"
        );
        commit(
            &mut plain,
            height,
            payments.get(&height).map_or(&[][..], Vec::as_slice),
        );
    }
    ...
    drop(plain);
    assert!(
        bytes(&plain_path) == physical,
        "instrumented and unrecorded storage bytes differ"
    );
    ...
    assert!(
        Instant::now() < deadline,
        "native profiling replay exceeded budget"
    );
