...
impl TestGenesis {
    pub fn generate(allocations: &[(Address, u64)]) -> Result<Self, PoolError> {
        ...
        for (address, value) in allocations {
            ...
            for _ in 0..128 {
                ...
                OsRng
                    .try_fill_bytes(&mut raw_rho)
                    .map_err(|_| PoolError::Genesis)?;
                OsRng
                    .try_fill_bytes(&mut raw_seed)
                    .map_err(|_| PoolError::Genesis)?;
                let Some(rho) = Option::<Rho>::from(Rho::from_bytes(&raw_rho)) else {
                    continue;
                };
                let Some(seed) = Option::<RandomSeed>::from(RandomSeed::from_bytes(raw_seed, &rho))
                else {
                    continue;
                };
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, PoolError> {
        ...
        for entry in bytes[HEADER..].as_chunks::<ENTRY>().0 {
            ...
            let value =
                u64::from_be_bytes(entry[43..51].try_into().map_err(|_| PoolError::Genesis)?);
            if value == 0 || value > TEST_SUPPLY {
                return Err(PoolError::Genesis);
            }
            total = total.checked_add(value).ok_or(PoolError::Genesis)?;
            ...
            if !rhos.insert(rho_bytes) {
                return Err(PoolError::Genesis);
            }
            let seed = Option::<RandomSeed>::from(RandomSeed::from_bytes(
                entry[83..115].try_into().map_err(|_| PoolError::Genesis)?,
                &rho,
            ))
            .ok_or(PoolError::Genesis)?;
    ...
    fn commitments(&self) -> Vec<Hash> {
        self.notes
            .iter()
            .map(|n| ExtractedNoteCommitment::from(n.commitment()).to_bytes())
            .collect()
    }
    ...
    pub fn read_pinned(path: &Path, expected: Hash) -> Result<Self, PoolError> {
        ...
        if !info.file_type().is_file()
            || info.len() > MAX_GENESIS_BYTES as u64
            || expected == [0; 32]
        {
            return Err(PoolError::Genesis);
        }
        let file = OpenOptions::new()
            ...
            .open(path)
            .map_err(|_| PoolError::Storage)?;
        if !file.metadata().map_err(|_| PoolError::Storage)?.is_file() {
            return Err(PoolError::Genesis);
        }
        let mut bytes = Vec::with_capacity(MAX_GENESIS_BYTES);
        file.take(MAX_GENESIS_BYTES as u64 + 1)
            .read_to_end(&mut bytes)
            .map_err(|_| PoolError::Storage)?;
        if Hash::from(Sha256::digest(&bytes)) != expected {
            return Err(PoolError::Genesis);
        }
        Self::decode(&bytes)
    }
...
mod tests {
    ...
    fn fixed_supply_and_canonical_openings() {
        ...
        for n in 0..g.bytes().len() {
            assert!(TestGenesis::decode(&g.bytes()[..n]).is_err());
        }
        let mut trailing = g.bytes().to_vec();
        trailing.push(0);
    ...
    fn different_manifests_bind_different_initial_states() {
        ...
        assert_ne!(
            a.initial_summary().unwrap().app_hash,
            b.initial_summary().unwrap().app_hash
        );
    }
}
