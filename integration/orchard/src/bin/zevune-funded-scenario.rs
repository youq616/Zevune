...
fn bad() -> io::Error { io::Error::new(io::ErrorKind::InvalidData, "funded laboratory protocol failure") }
fn ensure(ok: bool) -> Result<()> { if ok { Ok(()) } else { Err(bad().into()) } }
fn read_frame(r: &mut impl Read) -> Result<Option<Vec<u8>>> {
    ...
    let mut b = vec![0; n]; r.read_exact(&mut b)?; Ok(Some(b))
}
...
fn summary(s: &Summary) -> Vec<u8> {
    ...
    b.extend_from_slice(&s.app_hash); b.extend_from_slice(&s.root);
    for n in [s.commitments, s.nullifiers, s.fees] { b.extend_from_slice(&n.to_be_bytes()); }
    b
}
struct Scenario {
    root: PathBuf,
...
impl Scenario {
    fn new(root: &Path) -> Result<Self> {
        ...
        let mut wallets = Vec::new(); let mut passwords = Vec::new();
        for i in 0..3 {
            let mut password = Zeroizing::new([0; 32]); OsRng.try_fill_bytes(password.as_mut())?;
        ...
        let genesis = TestGenesis::generate(&[(w.receive_address(0)?, 50_000), (w.receive_address(1)?, 50_000)])?;
        genesis.write_new(&root.join("test-genesis.bin"))?;
        let pool = genesis.create_pool(&root.join("replay.journal"))?;
        let mut s = Self { root: root.into(), genesis, pool: Some(pool), wallets, passwords, prover: WalletProver::new(), phase: 0, backup_counter: 0 };
        s.sync_wallets()?; Ok(s)
    }
    fn sync_wallets(&mut self) -> Result<()> {
        let history = self.genesis.wallet_history(self.pool.as_mut().ok_or_else(bad)?)?;
        for w in &mut self.wallets { w.as_mut().ok_or_else(bad)?.sync(&history)?; }
        Ok(())
    }
    ...
    fn payment(&mut self, op: u8) -> Result<Vec<u8>> {
        ...
        let (sender, recipient, value) = if op == 1 {
            ensure(self.phase == 0)?; (0, 1, 60_000)
        } else { ensure(op == 3 && self.phase == 1)?; (1, 2, 40_000) };
        let destination = self.wallets[recipient].as_ref().ok_or_else(bad)?.view()?.receive_address(0)?;
        let expiry = self.pool.as_ref().ok_or_else(bad)?.summary()?.height.checked_add(100).ok_or_else(bad)?;
        let tx = self.wallets[sender].as_mut().ok_or_else(bad)?.prepare_payment(destination, value, 1_000, expiry, &self.prover)?;
        self.phase += 1;
        Ok(tx.bytes().to_vec())
    }
    fn apply(&mut self, raw: &[u8]) -> Result<Vec<u8>> {
        ...
        let mut p = 42; let mut txs = Vec::with_capacity(count);
        for _ in 0..count {
            let n = u32::from_be_bytes(raw.get(p..p+4).ok_or_else(bad)?.try_into()?) as usize; p += 4;
            ensure(n > 0 && n <= 28_102)?;
            let end = p.checked_add(n).ok_or_else(bad)?;
            txs.push(raw.get(p..end).ok_or_else(bad)?.to_vec()); p = end;
        }
        ensure(p == raw.len())?;
    ...
    fn restore(&mut self, index: usize) -> Result<Vec<u8>> {
        ...
        let path = self.root.join(format!("backup-{}-{index}.zwallet", self.backup_counter));
        let receipt = old.backup_new(&path)?;
        drop(self.wallets[index].take());
        self.wallets[index] = Some(WalletStore::open(&path, self.passwords[index].as_ref(), Some(receipt))?;
        self.sync_wallets()?;
        let restored = self.wallets[index].as_ref().ok_or_else(bad)?.pending_payment()?.map(|p| p.bytes().to_vec());
    ...
    fn finish(&mut self) -> Result<Vec<u8>> {
        ...
        for (i, expected) in [39_000, 19_000, 40_000].iter().enumerate() {
            ...
            ensure(wallet.balance()? == *expected && wallet.available_balance()? == *expected && wallet.pending_id().is_none())?;
        }
...
fn run() -> Result<()> {
    let args: Vec<_> = std::env::args_os().collect(); ensure(args.len() == 2)?;
    ...
    let mut r = stdin.lock(); let mut w = stdout.lock(); write_frame(&mut w, &ready)?;
    while let Some(request) = read_frame(&mut r)? {
        ...
        let mut response = vec![op]; response.extend_from_slice(&body); write_frame(&mut w, &response)?;
    }
    Ok(())
}
fn main() {
