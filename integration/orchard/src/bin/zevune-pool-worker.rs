...
fn frame(r: &mut impl Read) -> io::Result<Option<Vec<u8>>> {
    ...
    if !(1..=MAX_FRAME).contains(&n) { return Err(bad()); }
    let mut b = vec![0; n];
    r.read_exact(&mut b)?;
    Ok(Some(b))
}
fn write_frame(w: &mut impl Write, b: &[u8]) -> io::Result<()> {
    if b.is_empty() || b.len() > MAX_FRAME { return Err(bad()); }
...
fn block(data: &[u8]) -> Result<Block, PoolError> {
    ...
    if count > 16 { return Err(PoolError::Bounds); }
    let mut txs = Vec::with_capacity(count);
    for _ in 0..count {
        let n = u32::from_be_bytes(r.take()?) as usize;
        if n == 0 || n > MAX_ENVELOPE_SIZE || n > r.0.len() { return Err(PoolError::Bounds); }
        txs.push(r.0[..n].to_vec());
        r.0 = &r.0[n..];
    }
    if !r.0.is_empty() { return Err(PoolError::Bounds); }
    Ok((height, hash, txs))
}
...
struct Session { store: PoolStore, pending: Option<(Hash, PreparedBlock)> }
impl Session {
            ...
            1 | 2 => {
                ...
                if op == 2 {
                    if let Some((old, prepared)) = &self.pending {
                        return if old == &tag { Ok(prepared.result().clone()) } else { Err(PoolError::Stale) };
                    }
                }
                let prepared = self.store.prepare(height, hash, &txs)?;
                let out = prepared.result().clone();
                if op == 2 { self.pending = Some((tag, prepared)); }
                Ok(out)
            }
            3 if data.len() == 32 => {
                let Some((tag, _)) = &self.pending else { return Err(PoolError::Stale); };
                if data != tag { return Err(PoolError::Stale); }
...
fn serve(mut session: Session, r: &mut impl Read, w: &mut impl Write) -> io::Result<()> {
    ...
    while let Some(b) = frame(r)? {
        if b.len() < 17 || &b[..8] != b"ZVPLREQ1" { return Err(bad()); }
        let id = u64::from_be_bytes(b[8..16].try_into().map_err(|_| bad())?);
        if previous.checked_add(1) != Some(id) || b[16] > 4 { return Err(bad()); }
        previous = id;
        let result = session.apply(b[16], &b[17..]);
        // Storage uncertainty is fatal. EOF is not a negative transaction vote.
        if matches!(result, Err(PoolError::Storage | PoolError::Unavailable | PoolError::Corrupt | PoolError::Locked)) {
        ...
        let state = match result { Ok(s) => s, Err(_) => session.store.summary().map_err(|_| bad())? };
...
fn run() -> io::Result<()> {
    ...
    if args.len() != 3 || !Path::new(&args[2]).is_absolute() { return Err(bad()); }
    let path = Path::new(&args[2]);
    let store = if args[1] == "create" { PoolStore::create(path) } else if args[1] == "open" { PoolStore::open(path) } else { return Err(bad()); }
        .map_err(|_| io::Error::other("pool could not be opened"))?;
    serve(Session { store, pending: None }, &mut io::stdin().lock(), &mut io::stdout().lock())
}
fn main() {
    if run().is_err() {
...
mod tests {
    ...
    fn block_decoder_bounds() {
        ...
        b.extend_from_slice(&[1; 32]); b.extend_from_slice(&0u16.to_be_bytes());
        assert!(block(&b).is_ok());
        for n in 0..b.len() { assert!(block(&b[..n]).is_err()); }
        b.push(0); assert!(block(&b).is_err());
        b.truncate(42); b[41] = 17; assert!(block(&b).is_err());
        b[41] = 1; b.extend_from_slice(&u32::MAX.to_be_bytes()); assert!(block(&b).is_err());
    }
    #[test]
    fn frame_truncation_and_bombs() {
        assert!(frame(&mut &[][..]).unwrap().is_none());
        for b in [vec![0], vec![0, 0, 0, 0], vec![255; 4], vec![0, 0, 0, 2, 1]] { assert!(frame(&mut b.as_slice()).is_err()); }
        let mut b = Vec::new(); write_frame(&mut b, b"test").unwrap();
