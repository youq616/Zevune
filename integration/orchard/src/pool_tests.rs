...
fn actual_two_hop_pool_commit_replay_and_corruption() {
    let dir = TestDir::new();
    let path = dir.file("pool.journal");
    let alice = key();
    let bob = key();
    let carol = key();
    ...
    let seed = [<[u8; 32]>::from(&ExtractedNoteCommitment::from(
        initial.commitment(),
    ))];
    ...
    let first = prove(
        ...
        w,
        &[
            (bfvk.address_at(0u32, Scope::External), 60_000),
            (afvk.address_at(0u32, Scope::External), 39_000),
        ],
        &ctx,
    );
    ...
    assert!(matches!(
        pool.prepare(1, [1; 32], &[raw.clone(), raw.clone()]),
    ...
    broken[crate::wire::HEADER_SIZE + 2 * crate::wire::ACTION_SIZE + 4] ^= 1;
    assert!(matches!(
        pool.prepare(1, [1; 32], &[broken]),
        Err(PoolError::Authorization)
    ));
    assert_eq!(pool.summary().unwrap(), initial_summary);
    let first_prepared = pool
    ...
    assert_eq!(
        (
            first_summary.height,
            first_summary.commitments,
            first_summary.nullifiers,
            first_summary.fees
        ),
        (1, 3, 2, 1_000)
    );
    ...
    pool.check_checkpoint(first_summary.height, first_summary.app_hash)
        .unwrap();
    ...
    let bob_position = leaves.len() + received[0].0;
    leaves.extend(
        first
            .actions()
            .iter()
    ...
    let second = prove(
        ...
        bob_note,
        bob_path,
        &[(cfvk.address_at(0u32, Scope::External), 59_000)],
        &ctx,
    );
    let raw2 = crate::wire::encode(&second, &ctx).unwrap();
    let fresh = State::from_genesis(&seed).unwrap();
    assert!(matches!(
        fresh.execute(1, [1; 32], &[raw.clone(), raw2.clone()], &pool.verifier),
        Err(PoolError::Anchor)
    ));
    let second_prepared = pool
        .prepare(2, [2; 32], std::slice::from_ref(&raw2))
        .unwrap();
    let final_summary = pool.commit(second_prepared).unwrap();
    assert_eq!(
        (
            final_summary.height,
            final_summary.commitments,
            final_summary.nullifiers,
            final_summary.fees
        ),
        (2, 5, 4, 2_000)
    );
    ...
    assert_eq!(c.len(), 1);
    assert_eq!(c[0].2.value().inner(), 59_000);
    drop(pool);
    let recovered = PoolStore::open_with_genesis(&path, &seed).unwrap();
    assert_eq!(recovered.summary().unwrap(), final_summary);
    drop(recovered);
    ...
    *changed.last_mut().unwrap() ^= 1;
    ...
    let proof_offset =
        header + 4 + 114 + 4 + crate::wire::HEADER_SIZE + 2 * crate::wire::ACTION_SIZE + 4;
    ...
    assert!(matches!(
        PoolStore::open_with_genesis(&dir.file("forged"), &seed),
        Err(PoolError::Authorization)
    ));
    for length in [0, 1, header - 1, header + 1, original.len() - 1] {
        let damaged = dir.file(&format!("truncated-{length}"));
    ...
    let prefix = dir.file("valid-prefix");
    fs::write(&prefix, &original[..header + 4 + n + 32]).unwrap();
    let rolled = PoolStore::open_with_genesis(&prefix, &seed).unwrap();
    assert!(rolled
        .check_checkpoint(final_summary.height, final_summary.app_hash)
        .is_err());
...
fn empty_pool_lock_restart_and_no_overwrite() {
    let d = TestDir::new();
    let p = d.file("empty");
    let mut s = PoolStore::create(&p).unwrap();
    ...
    assert!(PoolStore::create(&p).is_err());
    assert_eq!(fs::metadata(&p).unwrap().len(), initial_len);
    assert!(matches!(PoolStore::open(&p), Err(PoolError::Locked)));
    let plan = s.prepare(1, [1; 32], &[]).unwrap();
    let after = s.commit(plan).unwrap();
    drop(s);
...
fn partial_write_poison_does_not_advance_memory_or_repair_disk() {
    let d = TestDir::new();
    let p = d.file("partial");
    let mut s = PoolStore::create(&p).unwrap();
    let before = s.state.summary();
    let plan = s.prepare(1, [1; 32], &[]).unwrap();
    s.fault = 1;
    assert_eq!(s.commit(plan), Err(PoolError::Storage));
    assert_eq!(s.state.summary(), before);
    assert_eq!(s.summary(), Err(PoolError::Unavailable));
    assert!(matches!(
        s.prepare(1, [1; 32], &[]),
        Err(PoolError::Unavailable)
    ));
    drop(s);
    let damaged = fs::read(&p).unwrap();
    assert!(PoolStore::open(&p).is_err());
    assert_eq!(fs::read(&p).unwrap(), damaged);
}
#[test]
fn lost_sync_acknowledgement_remains_uncertain_until_replay() {
    let d = TestDir::new();
    let p = d.file("uncertain");
    let mut s = PoolStore::create(&p).unwrap();
    let before = s.state.summary();
    let plan = s.prepare(1, [1; 32], &[]).unwrap();
    let next = plan.result().clone();
    s.fault = 2;
    assert_eq!(s.commit(plan), Err(PoolError::Storage));
    assert_eq!(s.state.summary(), before);
    assert_eq!(s.summary(), Err(PoolError::Unavailable));
...
fn tampered_prepared_result_and_resource_limits() {
    ...
    let length = fs::metadata(&p).unwrap().len();
    let mut plan = s.prepare(1, [1; 32], &[]).unwrap();
    plan.result.fees += 1;
    assert_eq!(s.commit(plan), Err(PoolError::Stale));
    assert_eq!(s.summary().unwrap(), before);
    assert_eq!(fs::metadata(&p).unwrap().len(), length);
    assert!(matches!(s.prepare(0, [1; 32], &[]), Err(PoolError::Height)));
    assert!(matches!(s.prepare(1, [0; 32], &[]), Err(PoolError::Height)));
    assert!(matches!(
        s.prepare(1, [1; 32], &vec![vec![]; MAX_BLOCK_TRANSACTIONS + 1]),
        Err(PoolError::Bounds)
    ));
    assert!(matches!(
        s.prepare(1, [1; 32], &[vec![0; MAX_ENVELOPE_SIZE + 1]]),
        Err(PoolError::Bounds)
    ));
...
fn record_decoder_rejects_truncation_trailing_and_length_bombs() {
    ...
    bomb[114..118].fill(255);
    assert!(Record::decode(&bomb).is_err());
