...
impl Dir {
    fn new() -> Self {
        let mut random = [0; 16]; OsRng.fill_bytes(&mut random);
...
fn commit(pool: &mut PoolStore, height: u64, txs: &[Vec<u8>]) -> Summary {
    let mut id = [0; 32]; id[..8].copy_from_slice(&height.to_be_bytes());
...
fn wallet_nonzero_multinote_payment_change_expiry_backup_and_rescan() {
    ...
    let mut password = zeroize::Zeroizing::new([0; 32]); OsRng.fill_bytes(password.as_mut());
    ...
    for w in [&mut bob, &mut carol, &mut dave] {
        w.sync(&origin).unwrap(); assert_eq!(w.balance().unwrap(), 0);
    }
    ...
    let funding = fixtures::prove(&pk, &issuer, initial, witness, &[
    ...
    bob.sync(&h1).unwrap(); carol.sync(&h1).unwrap();
    assert_eq!(bob.balance().unwrap(), 60_000);
    assert_eq!(carol.balance().unwrap(), 0);
    let prover = WalletProver::new();
    assert!(matches!(bob.build_payment(carol.receive_address(0).unwrap(), 60_000, 1_000, 20, &prover), Err(WalletError::InsufficientFunds)));
    assert!(bob.pending_id().is_none());
    assert!(matches!(bob.build_payment(carol.receive_address(0).unwrap(), 1, 0, 20, &prover), Err(WalletError::Bounds)));
    let pay = bob.build_payment(carol.receive_address(0).unwrap(), 50_000, 1_000, 20, &prover).unwrap();
    ...
    assert_eq!(bob.pending_id(), Some(pay.id()));
    assert_eq!(bob.available_balance().unwrap(), 0);
    assert!(matches!(bob.build_payment(carol.receive_address(0).unwrap(), 1, 1_000, 20, &prover), Err(WalletError::Pending)));
    let mut broken = pay.bytes().to_vec(); *broken.last_mut().unwrap() ^= 1;
    assert!(pool.prepare(2, [7; 32], &[pay.bytes().to_vec(), broken]).is_err());
    assert_eq!(pool.summary().unwrap(), first);
    commit(&mut pool, 2, &[pay.bytes().to_vec()]);
    let h2 = pool.wallet_history().unwrap();
    bob.sync(&h2).unwrap(); carol.sync(&h2).unwrap();
    assert!(bob.pending_id().is_none());
    ...
    let expires = recovered_carol.build_payment(dave.receive_address(0).unwrap(), 40_000, 1_000, 3, &prover).unwrap();
    commit(&mut pool, 3, &[]);
    recovered_carol.sync(&pool.wallet_history().unwrap()).unwrap();
    ...
    assert!(matches!(pool.prepare(5, [5; 32], &[expires.bytes().to_vec()]), Err(PoolError::Expired)));
    let onward = recovered_carol.build_payment(dave.receive_address(0).unwrap(), 40_000, 1_000, 20, &prover).unwrap();
    let end = commit(&mut pool, 5, &[onward.bytes().to_vec()]);
    let full = pool.wallet_history().unwrap();
    bob.sync(&full).unwrap(); recovered_carol.sync(&full).unwrap(); dave.sync(&full).unwrap();
    assert_eq!((bob.balance().unwrap(), recovered_carol.balance().unwrap(), dave.balance().unwrap(), end.fees), (9_000, 9_000, 40_000, 3_000));
    assert_eq!(39_000 + bob.balance().unwrap() + recovered_carol.balance().unwrap() + dave.balance().unwrap() + end.fees, 100_000);
    let file = dir.path("dave.vault"); vault::write_new(&file, &dave, password.as_ref()).unwrap();
    drop(dave); drop(pool);
    ...
    assert_eq!(dave.balance().unwrap(), 40_000);
    assert!(matches!(pool.prepare(6, [6; 32], &[onward.bytes().to_vec()]), Err(PoolError::DoubleSpend)));
    drop(pool);
...
fn wallet_rejects_changed_checkpoint_ancestry_without_mutation() {
    ...
    commit(&mut a, 1, &[]);
    let other = b.prepare(1, [9; 32], &[]).unwrap(); b.commit(other).unwrap();
    commit(&mut b, 2, &[]);
    ...
    assert_eq!(restored.sync(&b.wallet_history().unwrap()), Err(WalletError::Rollback));
    assert_eq!(wallet.sync(&b.wallet_history().unwrap()), Err(WalletError::Rollback));
