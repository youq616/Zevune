...
use zevune_orchard_lab::{
    signing_digest, Context, Error, StateView, Verifier, CIRCUIT, NETWORK, VERSION,
};
...
fn witness(leaves: &[MerkleHashOrchard], position: usize) -> (MerklePath, Anchor) {
    ...
    (
        MerklePath::from_parts(position as u32, path),
        nodes[0].into(),
    )
}
...
fn prove(
    ...
    for &(address, value) in outputs {
        ...
        b.add_output(None, address, NoteValue::from_raw(value), [0; 512])
            .unwrap();
    }
...
) -> Result<Signed, orchard::bundle::BundleError> {
...
fn corrupted_ciphertext(bundle: &Signed) -> Signed {
    ...
    actions.head = Action::from_parts(
        *a.nullifier(),
        a.rk().clone(),
        *a.cmx(),
        ciphertext,
...
fn upstream_public_empty_root_vectors() {
    ...
    for (i, bytes) in expected.iter().enumerate() {
        assert_eq!(
            &MerkleHashOrchard::empty_root(Level::from((i + 1) as u8)).to_bytes(),
            bytes
        );
    }
...
fn builder_rejects_wrong_owner_and_nonmember_path() {
    ...
    .unwrap();
    assert!(b
        .add_spend(FullViewingKey::from(&outsider), note, path.clone())
        .is_err());
    let mut b = Builder::new(
        BundleType::DEFAULT,
        VERSION,
...
fn real_proofs_two_hops_and_adversarial_cases() {
    ...
    let (first, proof1_ms) = prove(
        &pk,
        &alice,
        initial,
        path.clone(),
        &[(b_addr, 60_000), (a_addr, 39_000)],
        &ctx,
    );
    assert_eq!(first.actions().len(), 2);
    assert_eq!(
        first.authorization().proof().as_ref().len(),
    ...
    assert!(first
        .decrypt_outputs_with_keys(&[FullViewingKey::from(&stranger).to_ivk(Scope::External)])
        .is_empty());
    ...
    bob_bytes.fill(0);
    assert!(
        first.decrypt_outputs_with_keys(&[
            FullViewingKey::from(&restored_bob).to_ivk(Scope::External)
        ])[0]
            .2
            == bob_note
    );
    ...
    foreign.network = "other-network".into();
    assert_eq!(
        verifier.verify(&first, &foreign, &view),
        Err(Error::Network)
    );
    let mut changed_expiry = ctx.clone();
    changed_expiry.expiry_height += 1;
    assert_eq!(
        verifier.verify(&first, &changed_expiry, &view),
    ...
    assert_eq!(
        verifier.verify(&first, &ctx, &overflow),
        Err(Error::Expired)
    );
    ...
    let mut wrong_fee = ctx.clone();
    wrong_fee.fee += 1;
    assert_eq!(verifier.verify(&first, &wrong_fee, &view), Err(Error::Fee));
    wrong_fee.fee = u64::MAX;
    assert_eq!(verifier.verify(&first, &wrong_fee, &view), Err(Error::Fee));
    let changed_balance = first
        .clone()
        .try_map_value_balance(|v| Ok::<_, std::convert::Infallible>(v + 1))
        .unwrap();
    assert_eq!(
        verifier.verify(&changed_balance, &ctx, &view),
        Err(Error::Fee)
    );
    let matching_wrong_fee = context(1_001);
    assert_eq!(
        verifier.verify(&changed_balance, &matching_wrong_fee, &view),
        Err(Error::Authorization)
    );
    ...
    let broken = rebuild(
        ...
        Authorized::from_parts(
            Proof::new(proof),
            first.authorization().binding_signature().clone(),
        ),
        *first.anchor(),
        VERSION,
    )
    .unwrap();
    assert_eq!(
        verifier.verify(&broken, &ctx, &view),
        Err(Error::Authorization)
    );
    ...
    let unsigned = first.clone().map_authorization(
        &mut (),
        |_, _, _| Signature::<SpendAuth>::from([0; 64]),
        |_, auth| auth,
    );
    assert_eq!(
        verifier.verify(&unsigned, &ctx, &view),
        Err(Error::Authorization)
    );
    let broken_binding = rebuild(
        &first,
        Authorized::from_parts(
            first.authorization().proof().clone(),
            Signature::<Binding>::from([0; 64]),
        ),
        *first.anchor(),
        VERSION,
    )
    ...
    .unwrap();
    assert_eq!(
        verifier.verify(&anchor_changed, &ctx, &missing),
        Err(Error::Authorization)
    );
    println!(
        "M4_PHASE corrupted_proof_noncanonical_length_signatures_ciphertext_and_version: passed"
    );

    let mut repeated_actions = first.actions().clone();
    repeated_actions.tail[0] = repeated_actions.head.clone();
    let duplicate_action = Bundle::try_from_parts(
        repeated_actions,
        *first.flags(),
        *first.value_balance(),
        *first.anchor(),
    ...
    .unwrap();
    assert_eq!(
        verifier.verify(&duplicate_action, &ctx, &view),
    ...
    assert_eq!(
        verifier.verify(&first, &ctx, &repeated_output),
    ...
    let (unbacked, proof_negative_ms) = prove(
        &pk,
        &alice,
        initial,
        path,
    ...
    assert!(unbacked
        .verify_proof(&orchard::circuit::VerifyingKey::build(CIRCUIT))
        .is_ok());
    assert_eq!(
        verifier.verify(&unbacked, &context(0), &view),
        Err(Error::Fee)
    );
    ...
    assert_eq!(
        verifier.verify(&first, &ctx, &after_first),
        Err(Error::DoubleSpend)
    );
    ...
    assert!(second
        .decrypt_outputs_with_keys(&[afvk.to_ivk(Scope::External)])
        .is_empty());
