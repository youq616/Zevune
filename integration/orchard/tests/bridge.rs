...
use zevune_orchard_lab::{CIRCUIT,Verifier,StateView};
use zevune_orchard_lab::wire::{decode,encode,AuthorizationVerifier,ACTION_SIZE,HEADER_SIZE,MAX_ENVELOPE_SIZE};
use zevune_orchard_lab::worker;

#[test]
fn real_wire_roundtrip_individual_authorization_and_rejections() {
    let owner=key(); let recipient=key();
    ...
    let ctx=context();
    let bundle=prove(&pk,&owner,initial,path,&[
        (FullViewingKey::from(&recipient).address_at(0u32,Scope::External),60_000),
    ...
    assert!(Verifier::new().verify(&bundle,&ctx,&StateView{
        height:0,anchors:&[bundle.anchor().to_bytes()],spent:&BTreeSet::new(),outputs:&BTreeSet::new(),
    }).is_ok());
    for n in 0..raw.len(){assert!(decode(&raw[..n]).is_err());}
    let mut appended=raw.clone();appended.push(0);assert!(decode(&appended).is_err());
    assert!(decode(&vec![0;MAX_ENVELOPE_SIZE+1]).is_err());
    for offset in [HEADER_SIZE+2*ACTION_SIZE+4,HEADER_SIZE+820,raw.len()-1,15,23,HEADER_SIZE+160] {
        let mut altered=raw.clone();altered[offset]^=1;
        assert!(verifier.verify(&altered).is_err());
    }
    for offset in [32,HEADER_SIZE,HEADER_SIZE+32,HEADER_SIZE+64,HEADER_SIZE+96,HEADER_SIZE+128] {
        let mut altered=raw.clone();altered[offset..offset+32].fill(255);
        assert!(decode(&altered).is_err());
    }
    for offset in [HEADER_SIZE+64,HEADER_SIZE+128] {
        let mut altered=raw.clone();altered[offset..offset+32].fill(0);
        assert!(decode(&altered).is_err());
    }
    ...
    let mut input=Vec::new();worker::write_frame(&mut input,&request).unwrap();
    let mut output=Vec::new();worker::serve(&mut Cursor::new(input),&mut output).unwrap();
    let mut output=Cursor::new(output);
    let hello=worker::read_frame(&mut output,40).unwrap().unwrap();assert_eq!(hello.len(),40);
    let response=worker::read_frame(&mut output,49).unwrap().unwrap();
    assert_eq!(&response[..8],worker::RESPONSE_MAGIC);assert_eq!(response[16],0);assert_eq!(&response[17..],&digest);
    ...
    if let Some(dir)=std::env::var_os("ZEVUNE_PUBLIC_FIXTURE_DIR") {
        ...
        f.write_all(&raw).unwrap();f.sync_all().unwrap();
    }
}
