...
impl Verifier {
    ...
    fn authorize(&self, tx: &Transaction) -> Result<()> {
        ...
        for a in tx.bundle.actions().iter() {
            a.rk()
                .verify(&digest, a.authorization())
                .map_err(|_| Error::Authorization)?;
        }
        tx.bundle
            .binding_validating_key()
            .verify(&digest, tx.bundle.authorization().binding_signature())
            .map_err(|_| Error::Authorization)?;
        tx.bundle
...
impl Ledger {
    pub fn genesis(bytes: &[u8], verifier: &Verifier) -> Result<Self> {
        ...
        if tx.kind != 0
            ...
            || !tx.bundle.flags().outputs_enabled()
            || *tx.bundle.value_balance() != -(SUPPLY as i64)
            || *tx.bundle.anchor() != Anchor::empty_tree()
        {
            return Err(Error::Context);
        }
        verifier.authorize(&tx)?;
        let mut s = Self {
            genesis_bytes: bytes.to_vec(),
            genesis_id: Sha256::digest(bytes).into(),
            height: 0,
            last_block: [0; 32],
            leaves: vec![],
    ...
    pub fn check(&self, bytes: &[u8], at: u64, verifier: &Verifier) -> Result<Transaction> {
        let tx = Transaction::decode(bytes)?;
        if tx.kind != 1
            || tx.genesis != self.genesis_id
            || tx.bundle.flags() != &VERSION.default_flags()
            || tx.bundle.actions().len() < 2
            || tx.expiry < at
            || at == 0
        {
            return Err(Error::Context);
        }
        if tx.fee != FEE || *tx.bundle.value_balance() != FEE as i64 {
            return Err(Error::Funds);
        }
        if !self.anchors.contains(&tx.bundle.anchor().to_bytes()) {
    ...
    fn check_effects(&self, tx: &Transaction) -> Result<()> {
        ...
        for a in tx.bundle.actions().iter() {
            ...
            let cm = a.cmx().to_bytes();
            if self.spent.contains(&nf)
                || !nfs.insert(nf)
                || self.outputs.contains(&cm)
                || !cms.insert(cm)
            {
                return Err(Error::State);
            }
    ...
    pub fn preview(&self, block: &Block, verifier: &Verifier) -> Result<Self> {
        if self.height.checked_add(1) != Some(block.height)
            || self.history.len() >= MAX_BLOCKS
            || block.txs.len() > MAX_BLOCK_TXS
        {
            return Err(Error::Limit);
        }
        ...
        history.update(b"ZEVUNE-PAYMENT-LAB-HISTORY\0\x01");
        history.update(self.history_hash);
        history.update(block.height.to_be_bytes());
        history.update(hash);
        history.update((block.txs.len() as u32).to_be_bytes());
        for bytes in &block.txs {
            let raw = decode_tx_hex(bytes)?;
            candidate.history_bytes = candidate
                .history_bytes
                .checked_add(raw.len())
            ...
            candidate.burned = candidate
                .burned
                .checked_add(tx.fee)
                .filter(|v| *v <= SUPPLY)
                .ok_or(Error::Funds)?;
            history.update((raw.len() as u32).to_be_bytes());
            history.update(&raw);
        }
        let root = candidate.root();
        if candidate.anchors.last() != Some(&root) {
            candidate.anchors.push(root);
        }
        if candidate.anchors.len() > 64 {
            candidate.anchors.remove(0);
        }
        candidate.height = block.height;
        candidate.last_block = hash;
        candidate.history_hash = history.finalize().into();
        candidate.history.push(block.clone());
        Ok(candidate)
    }
    pub fn summary(&self) -> Summary {
        let mut h = Sha256::new();
        h.update(b"ZEVUNE-PAYMENT-LAB-STATE\0\x01");
        h.update(self.genesis_id);
        h.update(self.height.to_be_bytes());
        h.update(self.last_block);
        h.update(self.root());
        h.update((self.leaves.len() as u64).to_be_bytes());
        ...
        let state_hash: [u8; 32] = h.finalize().into();
        let mut app = Sha256::new();
        app.update(b"ZEVUNE-PAYMENT-LAB-APP\0\x01");
        app.update(state_hash);
        app.update(self.history_hash);
        Summary {
            chain_id: CHAIN.into(),
            genesis_id: hex::encode(self.genesis_id),
            height: self.height,
            app_hash: hex::encode(app.finalize()),
            state_hash: hex::encode(state_hash),
            history_hash: hex::encode(self.history_hash),
            note_root: hex::encode(self.root()),
            note_count: self.leaves.len(),
            spent_count: self.spent.len(),
            burned_fees: self.burned,
            test_asset_payments: true,
    ...
    pub fn export(&self) -> Export {
        Export {
            format: 1,
            genesis: hex::encode(&self.genesis_bytes),
    ...
    pub fn from_export(
        ...
        if export.format != 1
            || export.blocks.len() > MAX_BLOCKS
            || decode_tx_hex(&export.genesis)? != expected_genesis
        {
            return Err(Error::Context);
        }
        let mut s = Self::genesis(expected_genesis, verifier)?;
        for block in &export.blocks {
            s = s.preview(block, verifier)?;
        }
        if s.summary() != export.summary {
            return Err(Error::State);
        }
        Ok(s)
    }
    pub fn transactions(&self) -> Result<Vec<Transaction>> {
        ...
        for block in &self.history {
            for raw in &block.txs {
                txs.push(Transaction::decode(&decode_tx_hex(raw)?)?);
            }
        }
        Ok(txs)
    }
    pub fn witness(&self, position: usize, note: &Note) -> Result<MerklePath> {
        if position >= self.leaves.len()
            || self.leaves[position] != MerkleHashOrchard::from_cmx(&note.commitment().into())
        {
            return Err(Error::State);
        }
        ...
        if path.root(note.commitment().into()).to_bytes() != self.root() {
            return Err(Error::State);
        }
        Ok(path)
    }
