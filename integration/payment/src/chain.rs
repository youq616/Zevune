//! Deterministic bounded state. The signed app hash also binds exact history bytes.
...
pub struct Summary {
    ...
    pub app_hash: String, pub state_hash: String, pub history_hash: String,
    pub note_root: String, pub note_count: usize, pub spent_count: usize,
    pub burned_fees: u64, pub test_asset_payments: bool, pub real_funds_allowed: bool,
}
...
pub struct Ledger {
    ...
    outputs: BTreeSet<[u8; 32]>, anchors: Vec<[u8; 32]>, burned: u64,
    history: Vec<Block>, history_hash: [u8;32], history_bytes: usize,
}
impl Ledger {
    pub fn genesis(bytes: &[u8], verifier: &Verifier) -> Result<Self> {
        ...
        let mut s = Self { genesis_bytes: bytes.to_vec(), genesis_id: Sha256::digest(bytes).into(), height: 0, last_block: [0;32], leaves: vec![], frontier: Frontier::empty(), spent: BTreeSet::new(), outputs: BTreeSet::new(), anchors: vec![], burned: 0, history: vec![], history_hash:[0;32], history_bytes:0 };
    ...
    pub fn check(&self, bytes: &[u8], at: u64, verifier: &Verifier) -> Result<Transaction> {
        ...
        self.check_effects(&tx)?; verifier.authorize(&tx)?; Ok(tx)
    }
    fn check_effects(&self, tx: &Transaction) -> Result<()> {
        ...
        for a in tx.bundle.actions().iter() { let nf = a.nullifier().to_bytes(); let cm = a.cmx().to_bytes(); if self.spent.contains(&nf) || !nfs.insert(nf) || self.outputs.contains(&cm) || !cms.insert(cm) { return Err(Error::State); } } Ok(())
    }
    fn add_effects(&mut self, tx: &Transaction) -> Result<()> {
        self.check_effects(tx)?;
        for a in tx.bundle.actions().iter() { let leaf = MerkleHashOrchard::from_cmx(a.cmx()); if !self.frontier.append(leaf) { return Err(Error::Limit); } self.leaves.push(leaf); self.outputs.insert(a.cmx().to_bytes()); self.spent.insert(a.nullifier().to_bytes()); } Ok(())
    }
    pub fn preview(&self, block: &Block, verifier: &Verifier) -> Result<Self> {
        ...
        let hash = decode_hash(&block.hash)?; let mut candidate = self.clone();
        let mut history = Sha256::new(); history.update(b"ZEVUNE-PAYMENT-LAB-HISTORY\0\x01"); history.update(self.history_hash); history.update(block.height.to_be_bytes()); history.update(hash); history.update((block.txs.len() as u32).to_be_bytes());
        for bytes in &block.txs {
            let raw = decode_tx_hex(bytes)?;
            candidate.history_bytes = candidate.history_bytes.checked_add(raw.len()).filter(|n| *n <= MAX_HISTORY_BYTES).ok_or(Error::Limit)?;
            let tx = candidate.check(&raw, block.height, verifier)?; candidate.add_effects(&tx)?;
            candidate.burned = candidate.burned.checked_add(tx.fee).filter(|v| *v <= SUPPLY).ok_or(Error::Funds)?;
            history.update((raw.len() as u32).to_be_bytes()); history.update(&raw);
        }
        let root = candidate.root(); if candidate.anchors.last() != Some(&root) { candidate.anchors.push(root); }
        if candidate.anchors.len() > 64 { candidate.anchors.remove(0); }
        candidate.height = block.height; candidate.last_block = hash; candidate.history_hash=history.finalize().into(); candidate.history.push(block.clone()); Ok(candidate)
    }
    pub fn summary(&self) -> Summary {
        ...
        h.update((self.leaves.len() as u64).to_be_bytes()); h.update((self.spent.len() as u64).to_be_bytes()); for nf in &self.spent { h.update(nf); }
        h.update(self.burned.to_be_bytes()); h.update((self.anchors.len() as u64).to_be_bytes()); for root in &self.anchors { h.update(root); }
        let state_hash:[u8;32]=h.finalize().into(); let mut app=Sha256::new(); app.update(b"ZEVUNE-PAYMENT-LAB-APP\0\x01"); app.update(state_hash); app.update(self.history_hash);
        Summary { chain_id: CHAIN.into(), genesis_id: hex::encode(self.genesis_id), height: self.height, app_hash: hex::encode(app.finalize()), state_hash:hex::encode(state_hash), history_hash:hex::encode(self.history_hash), note_root: hex::encode(self.root()), note_count: self.leaves.len(), spent_count: self.spent.len(), burned_fees: self.burned, test_asset_payments: true, real_funds_allowed: false }
    }
    ...
    pub fn from_export(export: &Export, expected_genesis: &[u8], verifier: &Verifier) -> Result<Self> {
        ...
        let mut s = Self::genesis(expected_genesis, verifier)?; for block in &export.blocks { s = s.preview(block, verifier)?; }
        if s.summary() != export.summary { return Err(Error::State); } Ok(s)
    }
    pub fn transactions(&self) -> Result<Vec<Transaction>> { let mut txs = vec![Transaction::decode(&self.genesis_bytes)?]; for block in &self.history { for raw in &block.txs { txs.push(Transaction::decode(&decode_tx_hex(raw)?)?); } } Ok(txs) }
    pub fn witness(&self, position: usize, note: &Note) -> Result<MerklePath> {
        ...
        let mut nodes = self.leaves.clone(); let mut index = position; let mut path = [MerkleHashOrchard::empty_leaf();32];
        for (level, sibling) in path.iter_mut().enumerate() { let l = Level::from(level as u8); *sibling = nodes.get(index ^ 1).copied().unwrap_or_else(|| MerkleHashOrchard::empty_root(l)); nodes = nodes.chunks(2).map(|pair| MerkleHashOrchard::combine(l, &pair[0], &pair.get(1).copied().unwrap_or_else(|| MerkleHashOrchard::empty_root(l)))).collect(); index >>= 1; }
        let path = MerklePath::from_parts(position as u32, path); if path.root(note.commitment().into()).to_bytes() != self.root() { return Err(Error::State); } Ok(path)
    }
}
