...
pub fn signing_digest<A: Authorization>(
    ...
    if kind > 1 || bundle.bundle_version() != VERSION {
    ...
    h.update(b"ZEVUNE-PAYMENT-LAB-SIGNATURE\0\x01");
    h.update([kind]);
    h.update(genesis);
...
impl Transaction {
    pub fn encode(&self) -> Result<Vec<u8>> {
        ...
        let n = b.actions().len();
        if self.kind > 1
            || !(1..=MAX_ACTIONS).contains(&n)
            || b.bundle_version() != VERSION
            || b.authorization().proof().as_ref().len() != Proof::expected_proof_size(n)
        {
        ...
        out.push(n as u8);
        out.push(b.flag_byte());
        out.extend_from_slice(&b.value_balance().to_be_bytes());
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        if !(1..=MAX_ACTIONS).contains(&n) {
            return Err(Error::Limit);
        }
        let flags = match r.byte()? {
            2 => Flags::SPENDS_DISABLED,
            3 => VERSION.default_flags(),
            _ => return Err(Error::Encoding),
        };
        let balance = i64::from_be_bytes(r.array()?);
        let anchor =
            Option::<Anchor>::from(Anchor::from_bytes(r.array()?)).ok_or(Error::Encoding)?;
        ...
        for _ in 0..n {
            ...
            .ok_or(Error::Encoding)?;
            let encrypted = TransmittedNoteCiphertext {
                epk_bytes: r.array()?,
            ...
            actions.push(
                Action::from_parts(nf, rk, cm, encrypted, cv, signature)
                    .map_err(|_| Error::Encoding)?,
            );
        }
        let size = r.u32()? as usize;
        if size != Proof::expected_proof_size(n) {
            return Err(Error::Encoding);
        }
        let proof = Proof::new(r.take(size)?.to_vec());
        let binding = r.array::<64>()?.into();
        r.finish()?;
        let bundle = Bundle::try_from_parts(
            NonEmpty::from_vec(actions).ok_or(Error::Encoding)?,
            flags,
            balance,
            anchor,
        ...
        if tx.encode()?.as_slice() != bytes {
            return Err(Error::Encoding);
        }
        Ok(tx)
    }
    pub fn digest(&self) -> Result<[u8; 32]> {
        signing_digest(&self.bundle, self.kind, self.genesis, self.expiry, self.fee)
    }
}
pub struct Reader<'a> {
    bytes: &'a [u8],
...
impl<'a> Reader<'a> {
    ...
    pub fn take(&mut self, n: usize) -> Result<&'a [u8]> {
        ...
        let slice = self.bytes.get(self.pos..end).ok_or(Error::Encoding)?;
        self.pos = end;
        Ok(slice)
    }
