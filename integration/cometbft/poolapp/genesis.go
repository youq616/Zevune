...
func testGenesisState(digest poolbridge.Hash) []byte {
    if digest == (poolbridge.Hash{}) { return nil }
...
func validGenesisState(raw []byte, digest poolbridge.Hash) bool {
    if digest == (poolbridge.Hash{}) { return len(raw) == 0 }
    if len(raw) == 0 || len(raw) > 256 { return false }
    var binding genesisBinding
    d := json.NewDecoder(bytes.NewReader(raw))
    d.DisallowUnknownFields()
    if d.Decode(&binding) != nil || d.Decode(new(any)) != io.EOF { return false }
    ...
    if json.Compact(&compact, raw) != nil { return false }
