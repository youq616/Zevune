...
func (transport) RoundTrip(r *http.Request) (*http.Response,error) {
 res,err:=http.DefaultTransport.RoundTrip(r);if err==nil{res.Body=limitedBody{io.LimitReader(res.Body,48*1024*1024),res.Body}};return res,err
}
...
func hashBytes(text string) ([]byte,error) {
 if len(text)!=64{return nil,errors.New("invalid hash")};b,err:=hex.DecodeString(text);if err!=nil||hex.EncodeToString(b)!=text{return nil,errors.New("noncanonical hash")};return b,nil
}
...
func VerifyHistory(e paymentapp.Export,expectedGenesis []byte) error {
 ...
 g,err:=hex.DecodeString(e.Genesis);if err!=nil||!bytes.Equal(g,expectedGenesis){return errors.New("export genesis mismatch")}
 id:=sha256.Sum256(g);if e.Summary.GenesisID!=hex.EncodeToString(id[:]){return errors.New("wrong genesis ID")}
 ...
 for i,b:=range e.Blocks {
  ...
  for _,tx:=range b.Txs {
   if len(tx)>paymentapp.MaxTxBytes*2{return errors.New("transaction limit")};raw,err:=hex.DecodeString(tx);if err!=nil||hex.EncodeToString(raw)!=tx{return errors.New("invalid transaction bytes")}
   total+=len(raw);if total>8*1024*1024{return errors.New("history limit")};binary.BigEndian.PutUint32(count[:],uint32(len(raw)));_,_=h.Write(count[:]);_,_=h.Write(raw)
  };prior=h.Sum(nil)
 }
...
func ExportVerified(ctx context.Context,home string,index int) (paymentapp.Export,error) {
 ...
 genesis,err:=types.GenesisDocFromFile(cfg.DefaultConfig().SetRoot(filepath.Join(home,"node0")).GenesisFile());if err!=nil{return out,err}
 validators:=make([]*types.Validator,len(genesis.Validators));for i,v:=range genesis.Validators{validators[i]=types.NewValidator(v.PubKey,v.Power)};set:=types.NewValidatorSet(validators)
...
func receiptFor(e paymentapp.Export,tx []byte,already bool) (Receipt,bool) {
 ...
 for _,b:=range e.Blocks {for _,raw:=range b.Txs {if raw==want {
...
func Submit(ctx context.Context,home string,tx []byte) (Receipt,error) {
 var receipt Receipt
 if len(tx)==0||len(tx)>paymentapp.MaxTxBytes{return receipt,errors.New("invalid transaction size")};m,err:=Load(home);if err!=nil{return receipt,err};c,err:=Client(m,0);if err!=nil{return receipt,err}
 ...
 if broadcastErr!=nil||admitted.Code!=0 {
  ...
  if e,lookupErr:=ExportVerified(ctx,home,0);lookupErr==nil {if found,ok:=receiptFor(e,tx,true);ok{return found,nil}}
  if broadcastErr!=nil{return receipt,broadcastErr};return receipt,paymentapp.ErrRejected
 }
 ticker:=time.NewTicker(150*time.Millisecond);defer ticker.Stop()
 for {
  e,err:=ExportVerified(ctx,home,0)
  if err==nil {if found,ok:=receiptFor(e,tx,false);ok{return found,nil}}
