...
func Init(home string, basePort int) (Manifest, error) {
	...
	if home == "" || basePort < 1024 || basePort > 65528 {
		return m, errors.New("home and valid eight-port range required")
	}
	...
	if err = os.Mkdir(abs, 0700); err != nil {
		return m, fmt.Errorf("refusing existing or unavailable home: %w", err)
	}
	...
	for i := 0; i < NodeCount; i++ {
		c := cfg.DefaultConfig().SetRoot(nodeHome(abs, i))
		if err = genesis.SaveAs(c.GenesisFile()); err != nil {
			return m, err
		}
	}
...
func Load(home string) (Manifest, error) {
	...
	for _, id := range m.NodeIDs {
		...
		if er != nil || len(raw) != 20 || seen[id] {
			return m, errors.New("invalid/duplicate node ID")
		}
		seen[id] = true
	}
	for i := 0; i < NodeCount; i++ {
		...
		sum := sha256.Sum256(b)
		if hex.EncodeToString(sum[:]) != m.GenesisSHA256 {
			return m, errors.New("genesis mismatch; original files left unchanged")
		}
	}
	return m, nil
}
...
func Start(home string, index int, logger log.Logger) (*RunningNode, error) {
	...
	for _, p := range []string{c.PrivValidatorKeyFile(), c.PrivValidatorStateFile(), c.NodeKeyFile()} {
		...
		if !fi.Mode().IsRegular() {
			return nil, errors.New("validator files must be regular files")
		}
	}
	...
	if string(nk.ID()) != m.NodeIDs[index] {
		_ = a.Close()
		return nil, errors.New("node key does not match manifest")
	}
	...
	if !pub.Equals(genesis.Validators[index].PubKey) {
		_ = a.Close()
		return nil, errors.New("validator key does not match genesis")
	}
