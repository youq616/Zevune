...
func run() error {
	...
	if *action == "version" {
		fmt.Println("Zevune", app.Version, "CometBFT v0.38.26 NO-FUNDS")
		return nil
	}
	if *home == "" {
		return errors.New("-home is required; do not use an existing wallet or M1/M2 data directory")
	}
	...
	case "init":
		...
		if err == nil {
			fmt.Println("Initialized four local validators. NO PAYMENTS. Preserve all signing-state files.")
		}
		return err
	case "node":
		n, err := devnet.Start(absolute, *index, log.NewTMLogger(log.NewSyncWriter(os.Stderr)))
		if err != nil {
			return err
		}
	...
	case "start":
		...
		exits := make(chan error, devnet.NodeCount)
		stopped := false
		stop := func() {
			if stopped {
				return
