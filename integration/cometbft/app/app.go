...
func (a *Application) ready() error {
	if !a.engine.StorageStatus().Available {
		return ledger.ErrStorageUnavailable
	}
	return nil
}
...
func (a *Application) InitChain(_ context.Context, r *abci.RequestInitChain) (*abci.ResponseInitChain, error) {
	...
	if a.engine.Summary().Height != 0 || a.pending != nil {
		return nil, ledger.ErrHeight
	}
	if len(r.Validators) != 4 {
		return nil, errors.New("laboratory requires four genesis validators")
	}
	seen := make(map[string]bool)
	for _, v := range r.Validators {
		key := v.PubKey.GetEd25519()
		if v.Power != 10 || len(key) != 32 || seen[string(key)] {
			return nil, errors.New("invalid laboratory validator set")
		}
...
func (a *Application) PrepareProposal(_ context.Context, r *abci.RequestPrepareProposal) (*abci.ResponsePrepareProposal, error) {
	...
	if _, err := a.engine.PreviewBlock(uint64(r.Height), nil); err != nil {
		return nil, err
	}
...
func (a *Application) ProcessProposal(_ context.Context, r *abci.RequestProcessProposal) (*abci.ResponseProcessProposal, error) {
	...
	if r == nil || r.Height <= 0 || len(r.Txs) != 0 {
		return out, nil
	}
	if _, err := a.engine.PreviewBlock(uint64(r.Height), nil); err != nil {
		return out, nil
...
func (a *Application) FinalizeBlock(_ context.Context, r *abci.RequestFinalizeBlock) (*abci.ResponseFinalizeBlock, error) {
	...
	if r == nil || r.Height <= 0 || len(r.Hash) != 32 {
		return nil, errors.New("invalid finalization height/hash")
	}
	if len(r.Txs) != 0 {
