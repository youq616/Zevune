...
def parse_rust(lines):
    # Rust prints target announcements to stderr, whereas each test harness uses
    # stdout. A next target can therefore be announced before the prior result.
    # Cargo executes these targets sequentially; pair announcements and result
    # lines FIFO, never assign a result to simply the most recent announcement.
    waiting = deque()
    completed = []
    passed_names = []
    pending_test = None
    ...
    for number, line in enumerate(lines, 1):
        ...
        if match:
            waiting.append({"kind": "unit" if match[1] else "integration",
                            "target": match[2].replace("\\", "/"),
                            "announcement_line": number})
            continue
        if line.strip().startswith("Doc-tests "):
            waiting.append({"kind": "doc", "target": line.strip()[10:],
                            "announcement_line": number})
            continue
        ...
        if match:
            assert waiting and pending_test is None, line
    ...
    assert not waiting and pending_test is None and not passed_names and not totals
