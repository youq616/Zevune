...
工作流中的源码路径一律使用 Git 的正斜杠表示，与运行主机的路径分隔符无关；不转换文件名大小写。
来源回归在同一平台同时检查 Windows/POSIX 路径表示，并通过实际依赖收集逻辑验证 Windows 表示。
删去任一受测必需输入仍应失败，不能通过忽略整个工作流自身或缩减依赖集合获得通过。

恢复传输的继承回归另将“启动前预算耗尽”与“启动后进程等待超时”分开。后者保留20毫秒故障预算，
仅在测试内部暂停恢复模块的时钟，先执行真正的后端文件核验、创建真实子进程并确认两个读取线程
进入任务，再对原进程执行真实有界 wait，要求原 TimeoutExpired 原因及固定错误码。真实子进程、
标准库等待时钟和读取/关闭动作不被成功替身替代；被测试延迟的线程必须通过实际 join 释放。
另有慢文件核验场景，以及独立的20毫秒启动前耗尽拒绝场景。该控制时钟是测试设施，不改变生产
300秒调用限制、两秒读取线程收尾或五秒回收限制，也不是整项硬实时性能测量。
