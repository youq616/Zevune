...
源钱包在认证和采样前后核对同一文件身份、长度和完整公开链/摘要。文件必须与被采样的父目录
位于同一文件系统（st_dev 相同）；单独从另一文件系统挂载的钱包文件即使不是链接也明确拒绝，
不能借用父目录的余量给出绿色报告。源与目标的每次采样都绑定预先选定目录的设备/目录身份，
不是只检查一次查询内“恰好稳定”的替代目录；报告不输出这些可关联标识。目标目录也前后核对身份。
目标始终按其路径独立采样，即使设备和 inode 与源目录相同；绑定挂载别名可能有不同的只读标志，
不能用源目录的可写状态替代目标。此类别名回归使用受控元数据/只读标志，认证仍由真实Rust完成。
...
不当作通过认证的替代后端。跨文件系统钱包拒绝和采样目录替换回归使用元数据输入夹具，
不声称实际执行了特权挂载或模拟后端认证成功。原生端到端检查使用实际钱包和待发送付款，验证四计划不改变文件、
