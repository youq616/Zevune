...
无价值钱包，完成请求创建、确认签名、恢复相同pending、保留余额预留及错误密码/回执拒绝。
网络拒绝单独使用两个真实LAB2创世：直接调用原Rust操作码4验证错误创世摘要和跨网络收款地址；
再将正确固定摘要的另一网络请求/创世与原账本组合，经完整请求入口到达真实操作码4并拒绝。
三项均核对实际后端失败、没有新输出、两网络及钱包文件原字节和认证回执不变；Python预检
拒绝或后端启动失败不算通过这些原生场景。随后原网络的正常签名必须仍成功。
测试还故意丢弃一次**实际成功签名**的应用层回复，随后恢复原交易而非重签。这是测试驱动中断，
