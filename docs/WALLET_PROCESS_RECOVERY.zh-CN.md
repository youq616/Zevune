...
操作完成由测试专用观察文件标记；子进程还通过其自持文件句柄制作并关闭逐字节观察副本，
父进程不绕过 Windows 独占锁读取仍打开的钱包。强杀且确认退出后，再读取原文件与观察副本比较。
同进程钱包取证也使用自持句柄复制；账本物理字节只在关闭全部账本句柄之后读取，随后重新完整重放。
观察文件不是生产 CLI 回执，不能声称覆盖生产 CLI
...
两平台还必须确认本次实际原生 kill 请求成功，提前退出不能替代强杀。
...
强杀不会执行析构，也不承诺子进程的口令缓冲区在操作系统回收前主动清零。
