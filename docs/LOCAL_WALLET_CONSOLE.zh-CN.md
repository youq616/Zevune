...
控制台不广播、不自动重试、不访问 RPC 或远程节点。需要调用方持有可信的本地账本；不能在另一个节点占用同一账本文件时抢占其文件锁。不具备在线轻钱包同步、助记词、硬件签名、多设备并发、密码修改、自动原地压缩或生产部署能力；仅提供后文说明的显式新文件整理。
...
新增 `compact` 命令和后端操作码 10。必须提供当前源回执、明确确认，并选择不存在的目标路径；保留全部待发送交易和旧文件。成功后保存新回执、只启用新副本并重新同步。它不是余额/到账证明，不会广播。详见 [整理与故障边界](WALLET_COMPACTION.zh-CN.md)。
