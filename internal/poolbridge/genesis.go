...
func (o Options) workerArgs(mode string) ([]string, error) {
    ...
    if o.TestGenesis == "" && o.TestGenesisSHA256 == (Hash{}) { return args, nil }
    if !filepath.IsAbs(o.TestGenesis) || o.TestGenesisSHA256 == (Hash{}) { return nil, ErrBounds }
    info, err := os.Lstat(o.TestGenesis)
    if err != nil || !info.Mode().IsRegular() || info.Size() < 165 || info.Size() > 1890 { return nil, ErrBounds }
    f, err := os.Open(o.TestGenesis)
    if err != nil { return nil, ErrBounds }
    b, err := io.ReadAll(io.LimitReader(f, 1891))
    closeErr := f.Close()
    digest := sha256.Sum256(b)
    if err != nil || closeErr != nil || int64(len(b)) != info.Size() || !bytes.Equal(digest[:], o.TestGenesisSHA256[:]) { return nil, ErrBounds }
