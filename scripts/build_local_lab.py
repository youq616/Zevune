...
def build(destination: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="zevune-source-build-") as temporary:
        ...
        for name in ("LOCAL_NETWORK_OPERATOR.zh-CN.md", "WALLET_BACKUP_CATALOG.zh-CN.md", "WALLET_ARCHIVE.zh-CN.md", "PAYMENT_REQUESTS.zh-CN.md", "WALLET_HEALTH.zh-CN.md"):
            shutil.copy2(staged / "docs" / name, destination)
        result = manifest_for(destination, commit, versions, source_tree,
                              bundle_format="zevune-local-bundle-6")
