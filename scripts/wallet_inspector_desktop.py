...
class PasswordDialog:
    def __init__(self, app, intent):
        ...
        review = tk.Text(body, wrap="word", height=17, padx=8, pady=8)
...
class Workbench:
    def __init__(self, root):
        ...
        for row, (key, label) in enumerate(zip(FIELDS, labels)):
            ...
            entry = ttk.Entry(form, textvariable=self.values[key])
        ...
        self.details = tk.Text(output, wrap="word", state="disabled", height=10, padx=8, pady=8)
        ...
        ttk.Entry(output, textvariable=self.receipt, state="readonly").grid(row=2, sticky="ew")
