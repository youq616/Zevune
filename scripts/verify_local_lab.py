...
def required_files(windows: bool, version: int = 2) -> set[str]:
    ...
    if type(version) is not int or version not in (2, 3, 4, 5, 6, 7, 8, 9):
    ...
    if version >= 8:
        files |= {"zevune-pool-recovery" + suffix, "ledger_restore.py", "ledger_recovery_backend.py", "LEDGER_RESTORE.zh-CN.md"}
    if version == 9:
        files |= {"ledger_backup.py", "LEDGER_BACKUP.zh-CN.md"}
    return files
...
def verify(folder: Path, manifest_sha256: str, source_commit: str | None = None) -> dict[str, Any]:
    ...
    formats = {"zevune-local-bundle-2": 2, "zevune-local-bundle-3": 3, "zevune-local-bundle-4": 4, "zevune-local-bundle-5": 5, "zevune-local-bundle-6": 6, "zevune-local-bundle-7": 7, "zevune-local-bundle-8": 8, "zevune-local-bundle-9": 9}
