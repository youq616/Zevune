...
The fixture invokes the actual Python CLI and real recovery binary, then spends
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="zevune-chain-native-") as temporary:
        source = Path(temporary) / "source"
        export_build_source(root, commit, source)
        manifest = source / "integration/orchard/Cargo.toml"
        fixture = source / "scripts/fixtures/ledger_restore_cli.rs"
        target = source / "integration/orchard/tests/ledger_restore_cli.rs"
        if target.exists():
        ...
        subprocess.run(["cargo", "+1.98.1", "test", "--manifest-path", str(manifest), "--locked", "--release",
                        "--features", "local-funding-lab", "--test", "ledger_restore_cli", "--", "--test-threads=1"],
    ...
    print("Exact-source native ledger-chain recovery lifecycle completed; no validator readiness granted.")
