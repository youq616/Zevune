"""Execute the established real-Git source rejection tests on health CI."""
...
WORKFLOW = Path(__file__).resolve().parents[2] / '.github/workflows/wallet-health.yml'


class HealthSourceTests(established.CatalogSourceTests):
    def setUp(self):
    ...
    def test_workflow_pins_checkout_and_runs_source_regressions(self):
        ...
        for name in ('test_wallet_health.py', 'test_wallet_health_source.py'):
        ...
        self.assertIn("'scripts/check_wallet_health_backend.py', str(backend)", source)
