...
class DesktopSourceTests(established.CatalogSourceTests):
    ...
    def test_both_events_cover_transitive_local_test_dependencies(self):
        ...
        # A fixture-only edit must run the same headless/Tk/native matrix.
        repository = WORKFLOW.parents[2]
        scripts = repository / 'scripts'
        roots = (scripts, scripts / 'tests')
        pending = [scripts / name for name in
                   ('request_desktop.py', 'check_request_desktop_gui.py',
                    'check_request_desktop_backend.py')]
        pending += [scripts / 'tests' / name for name in
                    ('test_request_desktop.py', 'test_request_desktop_source.py',
